...
df = pd.read_csv('model_dev/data/processed/nypdshooting_data.csv')
len(df)
...
X = df.drop('vic_sex', axis=1)  # Features (all columns except 'vic_sex')
...
pickle.dump(X_train, open('model_dev/models/X_train.sav', 'wb'))
# Pkle X.columns for later use in explanation
pickle.dump(X.columns, open('model_dev/models/X_columns.sav', 'wb'))
